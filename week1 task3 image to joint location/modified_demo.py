...
def load_images_from_folder(folder):
    ...
    for filename in os.listdir(folder):
        ...
        if img is not None:
            images.append(img)
            imagescan.append(img)
            filenames.append(filename)
    return images,imagescan,filenames
images,imagescan,filenames = load_images_from_folder('sample_images/sampleframes')
print('number of images = ',len(images))
print('number of filenames = ',len(filenames))
savepath = 'C:/Users/M.Hemanth/AnacondaProjects/keras_Realtime_Multi-Person_Pose_Estimation-master/sample_images/annotated_frames'
...
for iter in range(len(images)):
    ...
    multiplier = [x * model_params['boxsize'] / images[iter].shape[0] for x in param['scale_search']]
    heatmap_avg = np.zeros((images[iter].shape[0], images[iter].shape[1], 19))
    paf_avg = np.zeros((images[iter].shape[0], images[iter].shape[1], 38))
    ...
    for m in range(len(multiplier)):
        scale = multiplier[m]
        imageToTest = cv2.resize(images[iter], (0,0), fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
        ...
        heatmap = cv2.resize(heatmap, (images[iter].shape[1], images[iter].shape[0]), interpolation=cv2.INTER_CUBIC)

        paf = np.squeeze(output_blobs[0]) # output 0 is PAFs
        paf = cv2.resize(paf, (0,0), fx=model_params['stride'], fy=model_params['stride'], interpolation=cv2.INTER_CUBIC)
        paf = paf[:imageToTest_padded.shape[0]-pad[2], :imageToTest_padded.shape[1]-pad[3], :]
        paf = cv2.resize(paf, (images[iter].shape[1], images[iter].shape[0]), interpolation=cv2.INTER_CUBIC)

        # visualization
        axarr2[m].imshow(images[iter][:,:,[2,1,0]])
        ax2 = axarr2[m].imshow(heatmap[:,:,3], alpha=.5) # right elbow
        axarr2[m].set_title('Heatmaps (Relb): scale %d' % m)

        axarr3.flat[m].imshow(images[iter][:,:,[2,1,0]])
        ax3x = axarr3.flat[m].imshow(paf[:,:,16], alpha=.5) # right elbow
        axarr3.flat[m].set_title('PAFs (x comp. of Rwri to Relb): scale %d' % m)
        axarr3.flat[len(multiplier) + m].imshow(images[iter][:,:,[2,1,0]])
    ...
    # 1
    plt.figure()
    plt.imshow(images[iter][:,:,[2,1,0]], alpha = .5)
    s = 5
        ...
        if(nA != 0 and nB != 0):
            ...
            for i in range(nA):
                for j in range(nB):
                    ...
                    score_with_dist_prior = sum(score_midpts)/len(score_midpts) + min(0.5*images[iter].shape[0]/norm-1, 0)
    ...
    canvas = imagescan[iter] # B,G,R order
    ...
    to_plot = cv2.addWeighted(images[iter], 0.3, canvas, 0.7, 0)
    ...
    fig = matplotlib.pyplot.gcf()
    fig.set_size_inches(12, 12)
    fig.savefig(savepath+'/'+filenames[iter])
